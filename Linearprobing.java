...
public class Linearprobing {
    ...
    public Linearprobing(int size, double load) {
        capacity = size;
        maxLoad = load;
    ...
    public void showSearchTimeStatistics() {
        ...
        for (int i = 0; i < searchCount; i++) {
            System.out.println("Search " + (i + 1) + ": " + searchTimes[i] + " ms");
        }
    }

    public static void main(String[] args) {
        String filePath = "/C:/users/liang/downloads/words_a-2.txt"; // File path
        int size = 61703; // Table size
        double load = 0.7; // Load factor

        Linearprobing table = new Linearprobing(size, load); // Initializes table
